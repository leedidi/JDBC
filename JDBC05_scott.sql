...
--VARCHAR로 잘못 생성... 테이블 삭제하기
--DROP TABLE TBL_BUSEO;
--==>> Table TBL_BUSEO이(가) 삭제되었습니다.
...
-- 위 삭제하고 다시 넣기(TRUNC)
--TRUNCATE TABLE TBL_JIKWI;

-- 시퀀스도 삭제하고 다시넣기^^,,,
--DROP SEQUENCE JIKWISEQ;
--==>> equence JIKWISEQ이(가) 삭제되었습니다.
...
--2) 전체 출력 코드 (사번/이름/부서이름/직위이름/(급여내림차순) 정렬)
-- 1. 사번순
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, (BASICPAY+SUDANG) AS TOTALPAY
FROM TBL_EMP
ORDER BY EMP_ID;
--> 한 줄 생성
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, (BASICPAY+SUDANG) AS TOTALPAY FROM TBL_EMP ORDER BY EMP_ID
;
...
ORDER BY EMP_NAME;
--> 한 줄 생성
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, (BASICPAY+SUDANG) AS TOTALPAY FROM TBL_EMP ORDER BY EMP_NAME
;
...
ORDER BY BUSEO_ID;
--> 한 줄 생성
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, (BASICPAY+SUDANG) AS TOTALPAY FROM TBL_EMP ORDER BY BUSEO_ID
;
...
ORDER BY JIKWI_ID;
--> 한 줄 생성
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, (BASICPAY+SUDANG) AS TOTALPAY FROM TBL_EMP ORDER BY JIKWI_ID
;
...
--> 한 줄 생성
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, (BASICPAY+SUDANG) AS TOTALPAY FROM TBL_EMP ORDER BY TOTALPAY DESC;
...
WHERE EMP_ID=1001;
--> 한 줄 생성
SELECT * FROM (SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, (BASICPAY+SUDANG) AS TOTALPAY FROM TBL_EMP) WHERE EMP_ID=1001
;
...
WHERE EMP_NAME='홍길동';
--> 한 줄 생성
SELECT * FROM (SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, (BASICPAY+SUDANG) AS TOTALPAY FROM TBL_EMP) WHERE EMP_NAME='홍길동'
;
...
WHERE BUSEO_ID=1;
--> 한 줄 생성
SELECT * FROM (SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, (BASICPAY+SUDANG) AS TOTALPAY FROM TBL_EMP) WHERE BUSEO_ID=1
;
...
WHERE JIKWI_ID=9;
--> 한 줄 생성
SELECT * FROM (SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, (BASICPAY+SUDANG) AS TOTALPAY FROM TBL_EMP) WHERE JIKWI_ID=9
;
...
--==>> 1004	김정훈	790304-1788896	2000-10-01	9	019-5236-4221	3	8	1954200	170000
UPDATE TBL_EMP
SET EMP_NAME='가가가', SSN='111111-1111111', CITY_ID=8, TEL='010-1111-1111', BUSEO_ID=1, JIKWI_ID=1, BASICPAY= 3333333, SUDANG = 111
WHERE EMP_ID = 1004;
--> 한 줄 생성
UPDATE TBL_EMP SET EMP_NAME='가가가', SSN='111111-1111111', CITY_ID=8, TEL='010-1111-1111', BUSEO_ID=1, JIKWI_ID=1, BASICPAY= 3333333, SUDANG = 111 WHERE EMP_ID = 1004
;
...
SELECT * FROM TBL_JIKWI WHERE JIKWI_NAME='대리';

--5. 데이터(직원 정보) 입력 쿼리문
/*
--(내 코드인데 오류남 어딘가 오타 존재한느낌)
INSERT INTO TBL_EMP(EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG)
...
--@ LEFT, RIGHT 조심... 참고할 거 비어있을때도 나오게하려면 이거 신경써야함!
--@ 날짜타입 컨트롤할 때는 조심조심! 
--@ -> TO_CHAR(E.IBSADATE, 'YYYY-MM-DD') AS IBSADATE.. 이래야 시분초까지 안나오고 입맛대로 필요한거만 쓰게할수있음
SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE AS IBSADATE
...
--@ 뷰 만들때는 기본적으로 조회 구문(뷰)에서는 다 가지고있게 만들고(소스 그대로 운용)
--@ 선택적으로 ... 필요한 데서 출력할때 특정 유형 데이터로 쓰겠다, 해서 사용하는게 좀 더 데이터의 가용성 향상시킴
-- TO_CHAR(E.IBSADATE,'YYYY-MM-DD')

--> 한 줄 구성
--CREATE OR REPLACE VIEW EMPVIEW AS SELECT E.EMP_ID , E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_NAME, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, E.BASICPAY, E.SUDANG, (E.BASICPAY+E.SUDANG) AS PAY FROM TBL_EMP E, TBL_CITY C, TBL_BUSEO B,TBL_JIKWI J WHERE E.CITY_ID = C.CITY_ID  AND B.BUSEO_ID = E.BUSEO_ID AND E.JIKWI_ID = J.JIKWI_ID
--;
--CREATE 구문이니까 필요 X... 걍 오라클에 있음!
...
--@ 사번순, 이름순, 부서순, 직위순 검색할때 총 XX 명 출력 되어야 하기 때문에 필요!
SELECT COUNT(*) AS COUNT
FROM EMPVIEW
WHERE EMP_ID=1001;
--WHERE EMP_NAME='정미화';
...
--10. 직원 데이터 수정 쿼리문
--@ 업무 로직에 준해서 수정 쿼리문 작성해보기
--@ 만들면, 미화 데이터 변경해보기
/*
UPDATE TBL_EMP
SET EMP_ID='1300', EMP_NAME='정미정', SSN='940000-2222222', IBSADATE = '2100-11-11'
  , CITY='서울', TEL='010-0000-0000', BUSEO_NAME='영업부', JIKWI_NAME='사원', BASICPAY; 
WHERE EMP_ID=1061;
*/
